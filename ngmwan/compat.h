#ifndef _MOREPIPE_COMPAT_H
#define _MOREPIPE_COMPAT_H
...
#error "Ngmwan requires Linux >= 3.10"
#endif
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 13, 0) && !defined(ISOPENSUSE15)
#define mp_newlink(a,b,c,d,e) mp_newlink(a,b,c,d)
#endif
...
#define __COMPAT_NETLINK_DUMP_BLOCK { \
	...
	ret = mp_get_device_dump_real(skb, cb); \
	skb->end += nlmsg_total_size(sizeof(int)); \
	return ret; \
}
#define __COMPAT_NETLINK_DUMP_OVERRIDE
#else
#define __COMPAT_NETLINK_DUMP_BLOCK return mp_get_device_dump_real(skb, cb);
#endif
...
#define mp_get_device_dump(a, b) mp_get_device_dump_real(a, b); \
static int mp_get_device_dump(a, b) { \
	struct mp_device *wg = (struct mp_device *)cb->args[0]; \
	if (!wg) { \
		int ret = mp_get_device_start(cb); \
		if (ret) \
			return ret; \
	} \
	__COMPAT_NETLINK_DUMP_BLOCK \
} \
static int mp_get_device_dump_real(a, b)
...
#define mp_expired_retransmit_handshake(a) mp_expired_retransmit_handshake(unsigned long timer)
#define mp_expired_send_keepalive(a) mp_expired_send_keepalive(unsigned long timer)
#define mp_expired_new_handshake(a) mp_expired_new_handshake(unsigned long timer)
#define mp_expired_zero_key_material(a) mp_expired_zero_key_material(unsigned long timer)
#define mp_expired_send_persistent_keepalive(a) mp_expired_send_persistent_keepalive(unsigned long timer)
#undef timer_setup
#define timer_setup(a, b, c) setup_timer(a, ((void (*)(unsigned long))b), ((unsigned long)a))
#undef from_timer
